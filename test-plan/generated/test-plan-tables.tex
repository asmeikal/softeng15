...
\subsection{Specifica dei Test Funzionali}\subsection{\code{TP\_1} - Verifica Saldo Attuale }
\label{sec:use-case:VERSAL}

\begin{ptable}{2}
\ptitle{Titolo}
\pcell{1}{
	Verifica Saldo Attuale (\code{TP\_VERSAL\_1})
}
\pline
\ptitle{Descrizione test}
\pcell{1}{
		Scopo del test \`e controllare il funzionamento dell'operazione di verifica del saldo attuale, come descritta dallo use case \iducVERSAL.
}
\pline
\ptitle{Prerequisiti}
\pcell{1}{
		L'utente $user$ possiede un account HBS registrato, e il suo saldo \`e $N$ (un valore numerico).
}
\pline
\ptitle{Azione}
\pcell{1}{
		\begin{enumerate} \item Effettuare il login come utente $user$; \item Navigare fino alla pagina di visualizzazione del saldo; \item Verificare che la pagina venga visualizzata correttamente e che la cifra visualizzata corrisponda a $N$ (ossia il valore presente nel back-end). \end{enumerate}
}
\end{ptable}

\subsection{\code{TP\_2} - Disposizione Pagamento }
\label{sec:use-case:DISPAG}

\begin{ptable}{2}
\ptitle{Titolo}
\pcell{1}{
	Disposizione Pagamento (\code{TP\_DISPAG\_2})
}
\pline
\ptitle{Descrizione test}
\pcell{1}{
...
\subsection{\code{TP\_3} - Verifica Login }
\label{sec:use-case:LOGIN}

\begin{ptable}{2}
\ptitle{Titolo}
\pcell{1}{
	Verifica Login (\code{TP\_LOGIN\_3})
}
\pline
\ptitle{Descrizione test}
\pcell{1}{
